...
if ($date != null) {
    if ($LeagueID != null) {
        $sql = "SELECT*FROM ketqua WHERE LeagueID=" . $LeagueID . " AND date_query=" . "'$date'" . " AND is_finish= 1";
    }
} else {
    $date = date('d/m/Y');
    if ($LeagueID != null) {
        $sql = "SELECT*FROM ketqua WHERE LeagueID=" . $LeagueID . " AND is_finish= 1";
    }
}
try {
    if ($LeagueID == null) {
...
if ($LeagueID != null) {
    $fp = fopen('C:/xampp/htdocs/ab/ktt/results/'.$id.'_vi.js', "w+");
    if (!$fp) {
